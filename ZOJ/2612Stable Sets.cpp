...
void dfs(int now, int flag) {
	if (mark) return;
	if (flag == 0) {
		for (int i = 0; i < adai1[now].size(); ++i) {
			int idx = s2[adai1[now][i]];
			if (idx == -1) {
				mark = 1;
				return;
			}
			...
			visited2[idx] = 1;
			for (int j = 0; j < adai2[idx].size(); ++j) {
				qs.push_back(adai2[idx][j]);
			}
			dfs(idx, 1 - flag);
		}
	} else {
		for (int i = 0; i < adai2[now].size(); ++i) {
			int idx = s1[adai2[now][i]];
			if (idx == -1) {
				mark = 1;
				return;
			}
			...
			visited1[idx] = 1;
			for (int j = 0; j < adai1[idx].size(); ++j) {
				ps.push_back(adai1[idx][j]);
			}
			dfs(idx, 1 - flag);
		}
	}
...
void run() {
	...
	s1.assign(n, -1); s2.assign(n, -1);
	for (int i = 0; i < size1; ++i) {
		for (int j = 0; j < adai1[i].size(); ++j) {
			s1[adai1[i][j]] = i;
		}
	}
	for (int i = 0; i < size2; ++i) {
		for (int j = 0; j < adai2[i].size(); ++j) {
			s2[adai2[i][j]] = i;
		}
	}
	visited1.assign(size1, 0); visited2.assign(size2, 0);
	ret = 0;
	vector<int> cc(size2, 0);
	for (int i = 0; i < size1; ++i) {
